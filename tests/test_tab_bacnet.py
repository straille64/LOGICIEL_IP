...
@pytest.fixture()
def mock_bac0():
    """Mock BAC0 pour éviter la dépendance au module lors des tests UI."""
    with patch("core.bacnet.BAC0") as m:
...
def test_tab_bacnet_builds(root, mock_bac0):
    from modules.tab_bacnet import TabBACnet
    tab = TabBACnet(root)
    assert tab.winfo_exists()


def test_tab_bacnet_has_client(root, mock_bac0):
    from modules.tab_bacnet import TabBACnet
...
def test_tab_bacnet_initial_status(root, mock_bac0):
    from modules.tab_bacnet import TabBACnet
    tab = TabBACnet(root)
    assert tab._var_status.get() == "Non connecté"


def test_bacnet_object_dialog_builds(root, mock_bac0):
